...
def create_medium_scenario():
    ...
    for i in range(1, 31):
        ...
        pickup_tw_end = pickup_tw_start + 120
        delivery_tw_start = pickup_tw_end + 20
        delivery_tw_end = delivery_tw_start + 120
...
def analyze_route(route, alns, label=""):
    ...
    print(f"\n{label}:")
    print(f"  服务任务数: {len(route.get_served_tasks())}/30")
...
def test_optimization_with_strategy(strategy_name, strategy, depot, tasks, distance_matrix, vehicle, energy_config, max_iterations=100):
    """测试单个充电策略的优化效果"""
...
def main():
    ...
    # 测试三种充电策略
    strategies = [
        ("完全充电 (FR)", FullRechargeStrategy(), 60),
        ("固定50% (PR-Fixed)", PartialRechargeFixedStrategy(charge_ratio=0.5), 75),
        ("最小充电 (PR-Minimal)", PartialRechargeMinimalStrategy(safety_margin=0.05, min_margin=0.0), 90)
    ]
    ...
    for r in results:
        ...
        print(f"  {r['strategy']:<20} {r['charging_visits_initial']:>3}次 → {r['charging_visits_optimized']:>3}次 "
              f"({visits_change:+d}次)")
    ...
    print(f"  优化耗时: {best_result['total_time']:.2f}秒")
