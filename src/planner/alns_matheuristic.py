...
class MatheuristicALNS(MinimalALNS):
    ...
    def optimize(self, initial_route: Route, max_iterations: int = 100) -> Route:
        ...
        repair_usage = {op: 0 for op in ("greedy", "regret2", "random")}
        ...
        self._log(
            "  Repair: "
            + ", ".join(f"{op}={repair_usage[op]}" for op in ("greedy", "regret2", "random"))
        )
        self._log(
            "  Destroy: "
    ...
    def _ensure_schedule(self, route: Route) -> bool:
        ...
        try:
            route.compute_schedule(
                distance_matrix=self.planner.distance,
                vehicle_capacity=vehicle.capacity,
                vehicle_battery_capacity=vehicle.battery_capacity,
                initial_battery=vehicle.initial_battery,
                time_config=getattr(self.planner, "time_config", None),
                energy_config=energy_config,
            )
        except Exception:  # pragma: no cover - defensive to keep optimisation running
            return False
