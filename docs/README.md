...
### 🧮 [MIP 基准模型 (MIP_BASELINE_MODEL.md)](./MIP_BASELINE_MODEL.md)
**← 规则选择/充电/冲突规避对照的数学基准模型**
...
- **规则库 + RL 选择**：将在线调度建模为可解释规则的选择式超启发式。
- **可学习部分充电**：离散化充电深度，便于稳定学习与策略对比。
- **冲突规避安全屏蔽**：基于头距的碰撞约束与冲突等待惩罚（w 汇聚节点/边延迟）。
- **MIP 基准模型**：用于小规模最优/下界对照，详见 [MIP_BASELINE_MODEL.md](./MIP_BASELINE_MODEL.md)。
...
- **能量约束**: 电池不耗尽，动态插入充电站（仿真层强制最低 SOC）
- **顺序约束**: Pickup必须先于Delivery

### 4. 多目标优化
```
最小化 = 距离成本 + 充电成本 + 迟到惩罚 + 冲突等待惩罚 + 拒单惩罚 + 驻留惩罚
```
